...
/**
 * getData:
 * Return every TPC-H query content graph in an array
 */
export async function getData() {
    let data: Graph[] = [];
    for (let i = 0; i < sql.length; i++) {
        let query = await requestQuery(i);
        if (query) data.push(query);
    }
    return data;
}
...
async function requestQuery(x: number) {
    ...
    function success(result: string) {
        ...
        return new Graph({ gid: x + 1, json });
    }
    function failure() {
        console.log('NETWORK FAILURE: LOADING TPC-H QUERY ' + (x + 1) + ' @ LOCAL PROJECT');
        let json = require(local + (x + 1) + '.json');
        return new Graph({ gid: x + 1, json });
    }
